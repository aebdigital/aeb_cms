...
export default function VavrostavCategories() {
  ...
  async function loadData() {
    setLoading(true)
    try {
      const nextCategories = await listEcommerceCategories()
  ...
  async function handleSubmitCategory(event) {
    ...
    try {
      if (editingCategory) {
        const updated = await updateEcommerceCategory(editingCategory.id, payload)
        setCategories(prev => prev.map(c => c.id === updated.id ? updated : c))
        showNotification('Kategória bola uložená', 'success')
      } else {
        const created = await createEcommerceCategory(payload)
  ...
  async function handleDeleteCategory(cat) {
    ...
    try {
      await deleteEcommerceCategory(cat.id)
              ...
              {categories.length === 0 ? (
                <tr>
                  <td colSpan="6" className="px-6 py-12 text-center text-sm text-gray-500">
                    Zatiaľ nemáte žiadne kategórie. Pridajte prvú kategóriu.
                  </td>
                </tr>
                ...
                <tr key={cat.id} className="transition hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-500 truncate max-w-[250px]">{cat.description || '-'}</div>
                  </td>
                  <td className="whitespace-nowrap px-6 py-4">
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                    <button onClick={() => openEditModal(cat)} className="mr-3 text-indigo-600 hover:text-indigo-900">
                      Upraviť
                    </button>
                    <button onClick={() => handleDeleteCategory(cat)} className="text-red-600 hover:text-red-900">
                      Vymazať
                    </button>
                  </td>
                </tr>
            ...
            <form onSubmit={handleSubmitCategory} className="p-6">
              <div className="space-y-4">
                ...
                <div className="mt-6 flex gap-3 justify-end pt-4 border-t border-gray-100">
                  <button type="button" onClick={closeModal} className="rounded-xl bg-gray-100 px-5 py-3 text-sm font-bold text-gray-700 hover:bg-gray-200 transition">
                    Zrušiť
                  </button>
                  <button type="submit" disabled={savingCategory} className="inline-flex items-center justify-center gap-2 rounded-xl bg-yellow-400 px-6 py-3 text-sm font-extrabold text-black transition hover:bg-yellow-300 disabled:opacity-60">

...
import { listBlogPosts, deleteBlogPost, togglePublishBlogPost } from '../api/lexanBlogs'
import LexanBlogEditor from './LexanBlogEditor'

const PREVIEW_ORIGIN = 'http://localhost:3006'

export default function LexanBlogsList() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Lexan blog</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Píšte články s 1:1 náhľadom podľa dizajnu lexan.sk.
          </p>
        </div>
        <button
      ...
      {editingId && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center !m-0">
          <div className="bg-white w-full max-w-[1400px] h-full sm:h-[96vh] sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col border sm:border-gray-200">
            <LexanBlogEditor editId={editingId} onClose={() => {
              setEditingId(null)
              load()
            }} />
          </div>

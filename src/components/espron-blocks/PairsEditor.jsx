...
export default function PairsEditor({ block, onChange }) {
  const title = block.title ?? ''
  const items = block.items ?? [{ title: '', body: '' }]
  ...
  function addItem() {
    onChange({ ...block, items: [...items, { title: '', body: '' }] })
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-3">
        <label className="block text-xs font-medium text-gray-600">Karty (nadpis + text)</label>
        {items.map((item, i) => (
